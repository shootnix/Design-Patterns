...
**Database abstraction:** ORM factory for PostgreSQL produces connection + query builder + schema migrator. Switch to MySQL by swapping the factory — everything else stays the same.
...
- You have `if $provider eq 'stripe' { ... } elsif $provider eq 'paypal' { ... }` in multiple places, each creating different objects
