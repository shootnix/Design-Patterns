...
**Payment processing:** Factory creates `PaymentGateway + RefundGateway + WebhookVerifier` as a matched set. Swap factories in one place; all handlers update automatically.

**Database abstraction:** ORM factory for PostgreSQL produces connection + query builder + schema migrator. Switch to MySQL by swapping the factory.

**Notification system:** Factory creates `Sender + TemplateRenderer + DeliveryTracker` per provider (SendGrid, Mailgun, SES).

## Signs you need this pattern

- You have `if $provider eq 'stripe' { ... } elsif ...` in multiple places, each creating different objects
